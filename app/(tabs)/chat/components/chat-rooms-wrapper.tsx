...
import {
  ...
  ChatRoomListProps,
  InitialChatRooms,
} from "@/app/(tabs)/chat/types";
import ChatRoomList from "@/app/(tabs)/chat/components/chat-room-list";
import { getMoreChatRooms } from "@/app/(tabs)/chat/repositories";
import { ArrowPathIcon } from "@heroicons/react/24/solid";
import NoChat from "@/app/(tabs)/chat/components/no-chat";
...
const ChatRoomsWrapper = ({
  ...
  const [chatRooms, setChatRooms] =
    useState<InitialChatRooms>(initialChatRooms);
    ...
    const observer = new IntersectionObserver(
      async (
        ...
        if (element.isIntersecting && trigger.current) {
          ...
          setIsLoading(true);
          const newChatRooms: ChatRoom[] = await getMoreChatRooms(
            page,
            user.id
          );
